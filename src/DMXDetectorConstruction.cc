...
G4VPhysicalVolume* DMXDetectorConstruction::Construct() {
  ...
  G4Box* solidAr2 = new G4Box("solidAr2", 0.5*A_l, 0.5*A_l, 0.5*A_l);
  logicAr2 = new G4LogicalVolume(solidAr2, LAr_mat, "logicAr2");  //sapphire_mat
  physAr2 = new G4PVPlacement(0, G4ThreeVector(0.,0.,0.5*(P_w+V_l+P_p)+0.5*(P_p-P_w)+2*m+0.5*A_l), logicAr2, "physAr2", logicWorld, false, 0);

  G4Box* solidAr3 = new G4Box("solidAr3", 0.5*A_l, 0.5*A_l, 0.5*A_l);
  logicAr3 = new G4LogicalVolume(solidAr3, LAr_mat, "logicAr3");  //sapphire_mat
  physAr3 = new G4PVPlacement(0, G4ThreeVector(0.,0.,0.5*(P_w+V_l+P_p)+0.5*(P_p-P_w)+1*m+0.5*A_l), logicAr3, "physAr3", logicWorld, false, 0);
